...
import glob
from mediapipe_function_4 import convert_landmarks_33, make_landmark_timestep
...
def pose_inference(video_file, frame_paths):
    ...
    for frame_path in tqdm(frame_paths):
        ...
        if results.pose_landmarks:
            ...
            kinect_v2_lm = convert_landmarks_33(lm, scale_factor=1)
...
V = 33  # Number of pose landmarks (nodes)

actions = ['cheer', 'clap', 'fall', 'jump', 'kick', 'run', 'sit', 'stand', 'throw', 'walk', 'wave']
video_files_dict = {}

for action in actions:
    file_list = glob.glob(f'D:/Documents/aHCMUT_Documents/DATN/NTU_DATASET/TRAIN_VAL_TEST_2/train/{action}/*.mp4') + glob.glob(f'D:/Documents/aHCMUT_Documents/DATN/NTU_DATASET/TRAIN_VAL_TEST_2/train/{action}/*.avi')
    video_files_dict[f'action{actions.index(action)}'] = sorted(file_list, key=os.path.getmtime)
...
np.save('D:/Documents/aHCMUT_Documents/DATN/NTU_DATASET/TRAIN_VAL_TEST_2/train_data_11action.npy', data)

# Create test_label.npy
label = np.repeat(np.arange(len(video_files_dict)), N // len(video_files_dict))
np.save('D:/Documents/aHCMUT_Documents/DATN/NTU_DATASET/TRAIN_VAL_TEST_2/train_label_11action.npy', label)
