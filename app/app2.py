...
from mediapipe_function_3 import make_landmark_timestep, convert_landmarks_25, plot_3d_landmarks
...
async def process_frames1(video_url):
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            kinect_v2_lm = convert_landmarks_25(lm, scale_factor=1)
            skeleton_state["current_skeleton"] = kinect_v2_lm
            # fig = plot_3d_landmarks(kinect_v2_lm)  # Assuming you have a function for 3D visualization
            ...
            # image_state["current_image"] = image_base64
            # Remove the oldest frame and append the new frame
            if len(lm_list) == T:
                regn_list = lm_list.copy()
                lm_list = lm_list[8:]
...
def generate_skeleton_image_base64():
    ...
    fig = plot_3d_landmarks(kinect_v2_landmarks)  # Assuming you have a function for 3D visualization
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:            
        label_data = {
            "label": shared_state["current_label"],
            "frame": frame_count,  # Replace frame_count with your current frame variable
        }
        await websocket.send_text(json.dumps(label_data))
        await asyncio.sleep(0.01)  # Adjust the sleep interval as needed
