...
from mediapipe_function_4 import make_landmark_timestep, convert_landmarks_25, plot_3d_landmarks_25
import base64
import websockets
import json
import asyncio
...
model = ST_GCN(num_classes=13, in_channels=3, t_kernel_size=9, hop_size=2)
model.load_state_dict(torch.load('model/13action25_100_64_0.1.pth', map_location=torch.device('cpu')))
model.eval()
...
action_labels = ['cheer', 'clap', 'drink', 'fall', 'jump', 'kick', 'pick', 'run', 'sit', 'stand', 'walk', 'wave', 'write']
...
# Gửi cảnh báo kèm hình ảnh qua telegram
...
# Hàm chính xử lý việc ghi nhận và đưa ra kết quả 
video_url = 0
...
def generate_skeleton_image_base64():
    ...
    fig = plot_3d_landmarks_25(kinect_v2_landmarks, elev, azim)  # Assuming you have a function for 3D visualization
    ...
    try:
        while True:
            ...
            if received_stream_url == 0:
                await asyncio.sleep(1.5)  # Adjust the delay as needed
            else:
...
if __name__ == "__main__":
    ...
    # loop.run_until_complete(process_frames(video_url))